/// Static product metadata, surfaced in the About dialog and sidebar.
class AppInfo {
  AppInfo._();

  static const String name = 'Helm';
  static const String version = '1.5.0';
  static const String author = 'Shakib';
  static const String tagline =
      'Take the helm of your Mac. A premium toolkit for a faster, cleaner machine — starting with storage.';
  static const String copyright = '© 2026 Shakib';
  static const String website = 'https://github.com/devshakib/helm';
}
